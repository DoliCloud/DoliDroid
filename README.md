...
## DESCRIPTION
This application is not a standalone program. It is a front end client to use an online hosted instance of Dolibarr ERP & CRM software (Open-source web software to manage your business, see https://www.dolibarr.org).
DoliDroid rewamps your Dolibarr web application pages, so all your online existing features are supported by this application. This is also true for external modules features.

The advantage of DoliDroid are :
- DoliDroid provides a menu system easier to use than the native web application.
- DoliDroid uses embedded image resources when available, to reduce bandwidth usage.
- DoliDroid uses internal cache for pages that should not change during session (like menu page)
...
* Provides a button, always available, to make a quick search on any item.
